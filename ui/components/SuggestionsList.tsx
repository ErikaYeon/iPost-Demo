...
const SuggestionsList: React.FC<SuggestionsListProps> = ({ suggestions, onSuggestionPress, theme }) => {
  return (
    <FlatList
      data={suggestions}
      keyExtractor={(item) => item.place_id || item.osm_id.toString()}
      renderItem={({ item }) => (
        <TouchableOpacity style={styles(theme).suggestionItem} onPress={() => onSuggestionPress(item.display_name)}>
