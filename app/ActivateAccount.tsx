...
import { useTranslation } from "react-i18next"; // Importación de traducción
...
const ActivateAccount: React.FC = () => {
  ...
  const handleReenvio = async () => {
    ...
    try {
      ...
      setSuccessMessage(t("activate_account.success_message")); // Mensaje traducido
    } catch (error) {
      console.error("Error from signup:", error);
      setErrorMessage(t("activate_account.error_message")); // Mensaje traducido
    }
  };
  ...
  return (
    <SafeAreaView style={sharedStyles.screenContainer}>
      ...
      >
        <HeaderText text={t("activate_account.title").split(" ")[0]} theme={theme} />
        <Text
          style={{
            fontSize: 34,
            fontWeight: "bold",
      ...
      <MessageText
        message={t("activate_account.message")} // Mensaje traducido
        boldText={t("activate_account.bold_message")} // Texto destacado traducido
        theme={theme}
      />

      {/* Botón "Reenviar enlace" */}
      <CustomButton
        title={t("activate_account.resend_link")} // Texto traducido
      ...
      <LinkText
        text={t("activate_account.go_to_login")} // Texto traducido
