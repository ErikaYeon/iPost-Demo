...
const Timeline: React.FC = () => {
  const { profileId, postId } = useLocalSearchParams(); // Obtén el ID del perfil y el post desde los parámetros
  ...
  const postHeights = useRef<{ [key: string]: number }>({}); // Guarda alturas por postId
  const [readyToScroll, setReadyToScroll] = useState(false); // Verifica si las alturas están listas

  const userProfilePosts = useSelector(
    (state: RootState) => state.profile.posts
  );

  const postIdAsString = Array.isArray(postId) ? postId[0] : postId; // Asegúrate de obtener el primer elemento si es un array
  ...
  useEffect(() => {
    if (readyToScroll && postIdAsString && userProfilePosts.length > 0) {
      const index = userProfilePosts.findIndex((post) => post.id === postIdAsString);
      if (index !== -1) {
        const screenHeight = Dimensions.get("window").height;
        const offsetToCenter =
          Object.keys(postHeights.current)
            .slice(0, index)
            .reduce((sum, id) => sum + (postHeights.current[id] || 0), 0) -
          (screenHeight - (postHeights.current[postIdAsString] || 0)) / 2;

        flatListRef.current?.scrollToOffset({
          offset: offsetToCenter > 0 ? offsetToCenter : 0,
          animated: true,
        });
      }
    }
  }, [readyToScroll, postIdAsString, userProfilePosts]);
          ...
          renderItem={({ item }) => (
            <View
              onLayout={(event) => {
                ...
                postHeights.current[item.id] = height; // Guarda la altura del post
                if (Object.keys(postHeights.current).length === userProfilePosts.length) {
                  setReadyToScroll(true); // Marca como listo cuando todas las alturas están registradas
                }
              }}
            >
              <Post
